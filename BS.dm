				...
				for(var/mob/G in world)
					if(G.client&&G)
						if(G.Genintest&&G)
							if(G.score>=10&&G)
								if(G)
									G<<"<font size=8><font color=red>Congratulations you have passed the genin test and are now a genin! - With [G.score] Points"
									world<<"Welcome the new Genin >> [G]"
									G.rank ="Genin"
									G.Genintest=0
									G.Geninmade=0
									G.score=0
	...
	proc
		Levelup()
			if(src.exp>=src.max_exp)
				src.level+=1
				src.exp -= 150
				src.max_exp+=rand(12,17)
				src.Mchakra+=rand(35,60)
				src.taiexp+=rand(15,20)
				src.ninexp+=rand(15,20)
				src.genexp+=rand(15,20)
				src<<"<font size = 2><b><font color = blue>You leveled up!!"
				src.maxhealth+=rand(40,70)
				src.taiup()
				src.ninup()
				src.genup()
				src.nin=src.Mnin
		...
		Death(mob/M)
			if(istype(src,/mob/pet)&&src.isdog&&!src.owned)
				...
				if(src.random==4)
					...
					if(src.isdog)
						for(var/mob/pet/P in world)
							if(P.owner == M)
								...
								P.health=0
								return
					world << "<font size=1><B>[src] has been killed by [M]!"
					src.deathforest=0
					if(src.earthscroll)
					...
					src<<"You will be brought back to your spawn point in 10 seconds!"
					sleep(80)
		...
		Enter()
			...
			if(istype(usr,/mob/))
				if(usr.client)
					...
					usr.onwater=0
					if(usr.leafS)
						usr.loc=locate(48,47,1)
					if(usr.sandS)
						usr.loc=locate(8,98,2)
					if(usr.grassS)
						usr.loc=locate(57,63,3)
					if(usr.soundS)
						usr.loc=locate(61,41,6)
					if(usr.rainS)
						usr.loc=locate(60,67,4)
					if(usr.earthS)
						usr.loc=locate(67,54,7)
					if(usr.lightningS)
						usr.loc=locate(24,57,8)
					if(usr.mistS)
						usr.loc=locate(34,64,9)
					if(usr.waterfallS)
						usr.loc=locate(58,36,24)
mob/var
	akatsukimember=0
	akatsukileader=0
	soundmember=0
...
mob/sound/verb/InviteS(mob/M in world)
	set category="Oranization"
	set name="Organization Invite"
	if(M.rank=="Jounin"&&M.Village!="Sound Organization")
