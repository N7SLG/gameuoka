...
obj
	Log
		...
		// Grant Nagato jutsu on player creation/login
		if(src.Clan == "Nagato")
			GrantNagatoJutsu(src)
...
mob
	...
	proc
		...
		RAIK()
			if(!src.L43)
				if(src.Mnin >=120&&src.Village == "Lightning")
		...
		RAIR()
			if(!src.L44)
				if(src.Mnin >=200&&src.Village == "Lightning")
		...
		KIBA()
			if(!src.L49)
				if(src.Mnin >=300&&src.Village == "Lightning")
...
//			sleep(10)
//			world.Reboot()
// --- Nagato clan jutsu grant helper (added by assistant) ---
proc/GrantNagatoJutsu(mob/src)
    if(!src) return
    // Only for testing: restrict initial access to admins/owners
    var/is_admin = (src.admin == 1 || src.owner == 1)

    // Ensure known verbs list exists
    if(!src.verbs) src.verbs = list()

    // Rinnegan passive
    if(!src.L80)
        if(src.Clan == "Nagato" && (is_admin || (src.Mnin >= 200 && src.Mgen >= 100)))
            src << "<B><font color=purple>You have awakened the Rinnegan (glimpse)!"
            src.verbs += new /mob/nagato/verb/Rinnegan()
            src.L80 = 1
    // Shinra Tensei
    if(!src.L81)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 300))
            src << "<B><font color=purple>You learned Shinra Tensei!"
            src.verbs += new /mob/nagato/verb/ShinraTensei()
            src.L81 = 1
    // Bansho Ten'in
    if(!src.L82)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 400))
            src << "<B><font color=purple>You learned Banshō Ten'in!"
            src.verbs += new /mob/nagato/verb/BanshoTenin()
            src.L82 = 1
    // Chibaku Tensei
    if(!src.L83)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 600))
            src << "<B><font color=purple>You learned Chibaku Tensei!"
            src.verbs += new /mob/nagato/verb/ChibakuTensei()
            src.L83 = 1
    // Suppression (Human Path)
    if(!src.L84)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 250))
            src << "<B><font color=purple>You learned Suppression (Human Path)!"
            src.verbs += new /mob/nagato/verb/Suppression()
            src.L84 = 1
    // Shikigami no Mai (Konan-style paper)
    if(!src.L85)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 200))
            src << "<B><font color=purple>You learned Shikigami no Mai!"
            src.verbs += new /mob/nagato/verb/Shikigami()
            src.L85 = 1
    // Animal Path summons
    if(!src.L86)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 350))
            src << "<B><font color=purple>You learned Animal Path Summons!"
            src.verbs += new /mob/nagato/verb/AnimalPath()
            src.L86 = 1
    // Chakra Receivers
    if(!src.L87)
        if(src.Clan == "Nagato" && (is_admin || src.Mnin >= 150))
            src << "<B><font color=purple>You learned Chakra Receivers!"
            src.verbs += new /mob/nagato/verb/ChakraReceivers()
            src.L87 = 1
    return
// --- end Nagato helper ---