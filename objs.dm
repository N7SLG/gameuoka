			...
			if(usr.cantest&&usr.rank=="Student")
				...
				switch(input("What are the Inuzuka clan known for?","Inuzuka ",) in list("Sharingan","Byakugan","Their dogs","The ability to control their bones","Their size"))
					if("Their dogs")
						usr.score += 1
						if(usr.score >= 9)
							usr<<"Congratulations you have passed the genin exam and are now a genin!"
							usr.rank ="Genin"
							usr.testing=0
							usr.Frozen=0
							return
...
obj/Niami1
    name = "NArmor"
    icon = 'NAKR.dmi' // placeholder base icon
    price = 250
    worn = 0
    var
        rNAKR = 0
    ...
    verb
        Wear()
            if(src.worn)
                ...
                usr.gen = src.base_gen
                usr.nin = src.base_nin

                usr << "You remove the [src.name]. Your energy settles."
                src.suffix = ""
            else
                // --- Equipping the armor ---
                src.worn = 1

                // store original stats before applying boosts
                src.base_gen = usr.gen
                src.base_nin = usr.nin

                // apply stat boost
                usr.gen = usr.gen * 5
                usr.nin = usr.nin * 5

                var/NAKRred = input(usr, "How much red do you want? (0–255)", "Customize Armor") as num
                var/NAKRgreen = input(usr, "How much green do you want? (0–255)", "Customize Armor") as num
                var/NAKRblue = input(usr, "How much blue do you want? (0–255)", "Customize Armor") as num

                // clamp to 0–255 range
                NAKRred = clamp(NAKRred, 0, 255)
                NAKRgreen = clamp(NAKRgreen, 0, 255)
                NAKRblue = clamp(NAKRblue, 0, 255)

                // create colored icon and apply tint
                var/icon/NAKRI = new('NAKR.dmi')
                NAKRI.Blend(rgb(NAKRred, NAKRgreen, NAKRblue), ICON_ADD)
                ...
                usr << "You wear the [src.name]. Power floods through your body!"
                src.suffix = "Equipped"

        Look()
            usr << "This is Niamis Gear — an armor radiating raw energy."
